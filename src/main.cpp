 ...
 * - Set Measurement intervall in seconds (1s-3600s)
 ***********************************************************/
...
#define T_SENSOR       "sensor"                   // Topic for Measurments
...
// Factory Defauzlt Settings
#define MEASURE_INTERVAL         30  // Measurement Interval 30s
...
boolean     g_lastDebug;
// Measure InterVal
uint32_t    g_measureInterval;             // seconds (0= never)
uint32_t    g_LastMeasurementDone;         // millis() when last measurement was done
 ...
 * - Return: `OK: New Interval: [NEWINTERVAL]s` 
 ************************************************************/ 
void cmd_interval(MyCommandParser::Argument *args, char *response) {  
  uint32_t myint;  
  String msgStr;  
  myint = (uint32_t) args[0].asUInt64;
  if (myint > 3600) {
    msgStr = "ERROR: Invalid Parameter: ";  
    msgStr.concat(myint);
    msgStr.concat(" (0-3600)");  
  } else {
    g_measureInterval = myint;
    msgStr = "OK: New Interval: ";  
    msgStr.concat(myint);    
    if (myint != 0){
      msgStr.concat("s");  
    } else {
...
void setupGlobalVars(void){
  ...
  g_measureInterval = MEASURE_INTERVAL;    // seconds (0= never)
  g_LastMeasurementDone = millis();        // millis() when last measurement was done
...
void setup(void) {  
  ...
  // ATTENTION "Commands" not more than 10 chars, see CommandParser.h
 ...
 * - has to check, weather a measurment should be done
 *   based on g_measureInterval
 ************************************************************/ 
void measureBME680(void) {
  String msgStr;    
  if (g_measureInterval > 0) {
    if ((millis() - g_LastMeasurementDone) > (g_measureInterval * 1000)) {
      g_LastMeasurementDone = millis();    
      // Measure 
      if (iaqSensor.run()) { 
        msgStr = '{';  
        msgStr.concat("\"temperature\":" + String(iaqSensor.temperature) + ",");
        msgStr.concat("\"relative humidity\":" + String(iaqSensor.humidity) + ",");           
        msgStr.concat("\"pressure\":" + String(iaqSensor.pressure) + ",");  
        msgStr.concat("\"gas\":" + String(iaqSensor.gasResistance) + ",");
        msgStr.concat("\"IAQ\":" + String(iaqSensor.iaq) + ",");
        msgStr.concat("\"IAQ accuracy\":" + String(iaqSensor.iaqAccuracy) + ",");
        msgStr.concat("\"Static IAQ\":" + String(iaqSensor.staticIaq) + ",");
        msgStr.concat("\"CO2 equivalent\":" + String(iaqSensor.co2Equivalent) + ",");  
        msgStr.concat("\"breath VOC equivalent\":" + String(iaqSensor.breathVocEquivalent) + ",");  
        msgStr.concat("\"raw temperature\":" + String(iaqSensor.rawTemperature) + ",");      
        msgStr.concat("\"raw relative humidity\":" + String(iaqSensor.rawHumidity) + ",");
        msgStr.concat("\"timestamp\":" + String(millis()) );
        msgStr.concat("}");  
        // Publish result
        mqttPub(T_SENSOR, msgStr, true);  
      } else {
        msgStr = "";
        // Error or warning        
        if (iaqSensor.status != BSEC_OK) {
          if (iaqSensor.status < BSEC_OK) {
            msgStr.concat("BSEC error code : " + String(iaqSensor.status));
          } else {
            msgStr.concat("BSEC warning code: " + String(iaqSensor.status));
          }
        }
        if (iaqSensor.bme680Status != BME680_OK) {
          if (iaqSensor.bme680Status < BME680_OK) {
            msgStr.concat("BME680 error code: " + String(iaqSensor.bme680Status));         
          } else {
            msgStr.concat("BME680 warning code : " + String(iaqSensor.bme680Status));      
          }
        }
        if (msgStr.length() > 0) {
